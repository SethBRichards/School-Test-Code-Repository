 ...
 *   
 *   This ino file uses a Data Driven Scheduler (DDS) to flash an LED while playing a tune
 *   and displaying that tune's frequency on a seven segment display in the process. While the tune is 
 *   off, a countdown until the next tune is played will be displayed.
 */
...
#define COUNTDOWN_INDEX     2  ///< Index of countdown timer task

#define NTASKS          10   ///< Number of total possible tasks in function pointer
#define DEFINED_TASKS   3    ///< Number of non null tasks in function pointer
...
void (*tasks[NTASKS])() = {task1_DDS,task2_DDS,task4_DDS,NULL};  // tasks to be entered into tasklist struct
void (*deadTasks[NTASKS])() = {NULL};                            // list of dead tasks (unused, for future efforts)

int ids[] = {LED_FLASH_INDEX,SONG_PLAYER_INDEX,COUNTDOWN_INDEX};                                   // list of unique task ID's in sequential order
String taskNames[] = {"LED Flash", "Song Player", "Countdown"};                                                 // list of unique task names in sequential order
 ...
 * While the song is being played, the frequency of the tone will be displayed on the 7 segment 
 * display.
 *
 * @return Returns nothing
 * @warning This function is not pre-emptive
...
void task2_DDS(){
  ...
  if(timet2 >= SPKR_ON_PERIOD_LONG && toneState == 4){ //silence for 4 sec
    ...
    toneState = 0;
    timet2=0;
    TaskList[COUNTDOWN_INDEX].state = READY;           // primes countdown task4 to start
  }
}


/**
 * @brief Displays a countdown until the tune in task 2 plays again
 *
 * task4_DDS() will countdown by 10ths of a second until task 2 begins again,
...
void task4_DDS(){
  ...
  static long int displayNum = SPKR_OFF_PERIOD/50;      //tracks ms number to display for countdown

  if(TaskList[task_index].state == READY){              //primes countdown to reset at beginning once per cycle
    displayNum = SPKR_OFF_PERIOD/50;
  }
